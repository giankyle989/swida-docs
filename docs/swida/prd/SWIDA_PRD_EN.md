...
>
> - §3.1 Dashboard capability description updated to match prototype stat cards and activity feeds
> - §7.2 Admin-Facing Menus updated: sidebar-only shell, revised group names, New Shop as sidebar item, Shops total-count badge, no top header "View Site" or "Admin ▾" dropdown
> - §7.3 unchanged (workflows unchanged)
> - No Customer Web sections modified
...
- **Dashboard:** Access to an admin dashboard showing platform statistics (total shops, monthly active users, total reviews, new partnership inquiries) with delta indicators, a verify-needed alert for shops not re-verified in 90 days, a pending reviews feed, recent shops feed, and recent inquiries feed — built as a dedicated page in the Admin Web (Next.js)
...
**Shell layout:** The Admin Web uses a sidebar-only shell. The left sidebar contains the logo mark, navigation links grouped by category, and the logged-in admin user identity (name + role) at the bottom. A topbar across the top of the main content area shows the current page title, breadcrumb, and contextual action buttons (e.g., "+ New Shop" on the shops page). There is no separate global header bar.
...
| -------------- | -------------------------------------------------------------------------------------------------------- |
| **Overview**   | Dashboard                                                                                                |
| **Content**    | Shops (with total shop count badge), New Shop, Themes, Locations                                         |
| **Moderation** | Reviews (with `under_review` count badge in yellow), Partnership (with `new` inquiry count badge), Users |
| **System**     | Audit Log                                                                                                |

**Admin user identity** is displayed at the bottom of the sidebar: avatar initial, display name, and role label. Logout is accessed from this area.

**Admin Web pages and sections:**

| Admin Web Page/Section | Description                                                                                                                                                                                        |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Dashboard**          | Platform overview: 4 stat cards (Total Shops, Monthly Users, Total Reviews, New Inquiries), verify-needed alert banner, pending reviews feed, recent shops feed, recent partnership inquiries feed |
| **Shops**              | CRUD interface for all shop listings with search, filter, and sort. "+ New Shop" button in topbar.                                                                                                 |
| **New Shop**           | Shortcut sidebar link navigating directly to `/shops/new`                                                                                                                                          |
| **Themes**             | View and edit service themes                                                                                                                                                                       |
| **Locations**          | Manage Level 1 and Level 2 location entries                                                                                                                                                        |
| **Reviews**            | View and moderate reviews (Keep / Hide actions)                                                                                                                                                    |
| **Partnership**        | Track and manage incoming partnership requests as cards with status workflow                                                                                                                       |
| **Users**              | Customer account management (lock/unlock)                                                                                                                                                          |
| **Audit Log**          | History of admin actions, filterable by change type                                                                                                                                                |

**Custom Admin Features:**

- **Dashboard:** Platform stats with delta indicators and activity feeds — built as a Next.js page with data fetched from a custom Strapi analytics endpoint (`GET /api/dashboard/stats`, TSD §5.2.5).
- **Verify-Needed Alert:** The dashboard prominently shows a banner when shops have not been re-verified in over 90 days (PRD §10.4), linking to the filtered shop list.
- **Audit Log Viewer:** History of all admin actions, displayed in a searchable, filterable flat table (no expand/collapse diffs in the prototype — flat row-per-change format). Data stored in Strapi's `audit-log` collection type, populated via lifecycle hooks.
