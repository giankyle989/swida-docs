...
| Inactive Reason Detail | `other`인 경우에만 | 자유 텍스트 설명 |
...
| F-SHOP-22 | 이미지 업로드 | 업체 이미지 업로드 (1–10장). 드래그 앤 드롭 또는 파일 선택. 이미지는 Strapi Upload → MinIO를 통해 저장 (PRD §5.1) |
...
| F-SHOP-27 | 로케일 전환기 | 한국어와 영어 콘텐츠 편집 간 전환. 한국어 우선 필수, 영어는 선택 (PRD §11.4) |
...
| F-THEME-04 | 테마 삭제 | 테마 삭제. 확인 대화상자 표시. 해당 테마에 태그된 업체가 없는 경우에만 허용. 업체가 존재하면 업체 수와 함께 경고 표시 |
...
| F-LOC-05 | 구역 목록 | 지역을 클릭하면 확장/이동하여 해당 Level 2 구역 표시. 테이블: 이름(한국어), 이름(영어), 업체 수 |
...
| F-INQ-07 | 상태 업데이트 | 드롭다운을 통한 문의 상태 변경. 상태 흐름: `new` → `contacted` → `awaiting_info` → `approved` → `published`. 또한: 모든 상태 → `rejected` (PRD §10.3.2) |
...
| F-INQ-12 | 신규 문의 배지 | 미읽은 `new` 문의 수를 나타내는 시각적 배지/카운터. 48시간 SLA 표시기 — 연락 없이 48시간이 경과한 문의 강조 표시 (PRD §10.3.3) |
